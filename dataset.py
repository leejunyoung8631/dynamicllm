...
def get_loaders2(name, tokenizer, seq_len=2048, batch_size=8, add_bos_to_every=False):
    if "wikitext2" in name:
        ...
        # subtrain_data = train_data.shuffle(seed=42)
        test_data = test_data.shuffle(seed=42)
        
        subtrain_dataset = process_data(
            test_data, tokenizer, seq_len, "text", add_bos_to_every
        )
        # test_dataset = process_data(
        #     test_data, tokenizer, seq_len, "text", add_bos_to_every
    ...
    test_loader = DataLoader(
        subtrain_dataset, batch_size=batch_size, shuffle=False
    )
    subtrain_loader = DataLoader(
        subtrain_dataset, batch_size=batch_size, shuffle=False
    )
    
    return subtrain_loader, test_loader