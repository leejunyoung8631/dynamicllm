...
class CustomLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        for n_layer, decoder_layer in enumerate(self.layers):
            ...
            # changed -> appiled normed hidden_state
...
class CustomLlamaForCausalLM(LlamaForCausalLM):
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # '''
            # nxt_token = []
            # layer_hidden = outputs.layer_hidden_states
            # for feature in layer_hidden:
            #     next_token_logits = feature.clone()[:, -1, :].float()
            #     next_token_logits = next_token_logits.to(input_ids.device)
            #     next_token_scores = logits_processor(input_ids, next_token_logits)
                
            #     next_tokens = torch.argmax(next_token_logits, dim=-1)
            #     nxt_token.append(next_tokens.item())
                
            # import csv
            # with open("tokeninfo_2.csv", mode='a', newline='') as file:
            #     writer = csv.writer(file)
            #     writer.writerow(nxt_token)
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        # temp, 27
        # skip_layer = [24, 26, 25, 10, 27, 13, 22, 14, 9, 29, 12]
        ...
        # import csv
        # layer_predict = ()
        # layer_hidden = outputs.hidden_states # 32, len_token, 4096 => ( torch.Tensor(1, 11, 4096) x 32)
        # for layer_feature in layer_hidden:
        #     layer_predict += (self.lm_head(layer_feature[:, -num_logits_to_keep:, :]), )  
        
        # nxt_token = []
        # for feature in layer_predict:
        #     next_token_logits = feature.clone()[:, -1, :].float()
        #     next_tokens = torch.argmax(next_token_logits, dim=-1)
        #     nxt_token.append(next_tokens.item())
            
        # with open("tokeninfo.csv", mode='a', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow(nxt_token)  
        ...
        return CustomCausalLMOutputWithPast(
            ...
            layer_hidden_states=layer_predict,
        )
